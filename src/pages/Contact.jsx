          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 bg-white p-8 md:p-12 rounded-3xl shadow-xl border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button type="submit" disabled={status === 'loading' || status === 'success'} className={`w-full py-4 rounded-xl transition font-semibold text-lg flex items-center justify-center text-white ${status === 'loading' ? 'bg-crystal-400' : status === 'success' ? 'bg-green-500' : 'bg-crystal-500 hover:bg-crystal-600'}`}>
                {status === 'loading' ? (<><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Sending...</>) : status === 'success' ? (<><CheckCircle className="w-5 h-5 mr-2" /> Request Sent!</>) : ('Send Request')}
              </button>

              {status === 'success' && <div className="bg-green-50 text-green-700 p-4 rounded-xl text-center text-sm">Thank you! Your request has been sent. Check your email for confirmation.</div>}
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-8">
            ...
            <div className="bg-gray-900 text-white p-8 rounded-3xl">
              ...
              <div className="space-y-4">
                <div className="flex items-start"><MapPin className="w-6 h-6 mr-4 text-crystal-400 flex-shrink-0" /><div><p className="font-medium">Office Location</p><p className="text-gray-400 text-sm">123 Sparkle Street, New York, NY 10001</p></div></div>
                <div className="flex items-start"><Phone className="w-6 h-6 mr-4 text-crystal-400 flex-shrink-0" /><div><p className="font-medium">Call Us</p><p className="text-gray-400 text-sm">(555) 123-4567</p></div></div>
