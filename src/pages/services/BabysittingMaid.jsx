...
export default function BabysittingMaid() {
  ...
  const handleAddToCart = () => {
    addToCart({ service: 'Babysitting & Maid', hours, crew, ratePerHour, totalAmount });
    setAdded(true);
  ...
  return (
    <div className="pt-20 pb-32 bg-white min-h-screen">
      <Helmet>
        <title>Babysitting & Maid Services in Dubai | Lucky Crystal Maids</title>
        <meta name="description" content="Trusted babysitters and maids in Dubai. Background-checked, reliable, and caring. Childcare, light housekeeping, and school runs. Book a sitter today!" />
      </Helmet>

      <div className="relative h-[400px] w-full overflow-hidden">
        <img src="https://images.unsplash.com/photo-1516627145497-ae6968895b74?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" alt="Dubai Maid and Babysitting" className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-2">Babysitting & Maid in Dubai</h1>
            <p className="text-xl text-gray-200 mb-6">Trusted care for your home and little ones.</p>
      ...
      <div className="container mx-auto px-6 max-w-6xl mt-16 grid md:grid-cols-3 gap-12">
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:col-span-2">
          <h2 className="text-3xl font-display font-bold text-gray-800 mb-6">Reliable Childcare & Household Help</h2>
          <p className="text-gray-600 mb-8 leading-relaxed">Finding someone you trust to care for your children and home in Dubai is hard. Our vetted professionals provide peace of mind with reliable, loving care. Whether you need a full-time maid in Arabian Ranches or a part-time sitter in Downtown Dubai, we've got you covered.</p>
          
          <div className="grid md:grid-cols-2 gap-4 mb-8">
            {['Verified & background-checked staff', 'Child meal preparation', 'Light housekeeping', 'School run & errands', 'Engaging playtime activities', 'Homework assistance'].map((f, i) => (
              <div key={i} className="flex items-start bg-indigo-50 p-4 rounded-xl"><CheckCircle className="w-6 h-6 text-indigo-500 mr-3 flex-shrink-0 mt-1" /><span className="text-gray-700">{f}</span></div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div id="booking-picker" initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8 scroll-mt-24">
          <div className="bg-indigo-50 p-8 rounded-3xl border border-indigo-100 sticky top-24">
            <h3 className="text-2xl font-bold text-gray-800 mb-6">Select Your Helper</h3>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Helpers (Min 1, Max 10)</label>
              <div className="flex items-center justify-between bg-white p-3 rounded-xl border border-gray-200">
                ...
                <span className="text-xl font-bold text-gray-800">{crew} Helper{crew > 1 ? 's' : ''}</span>
            ...
            <div className="bg-white p-4 rounded-xl mb-6 text-sm text-gray-600 space-y-1">
              ...
              <p>Calculation: {hours}h × {crew} crew × {ratePerHour} AED</p>
