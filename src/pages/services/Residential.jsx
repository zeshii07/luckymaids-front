...
import { CheckCircle, ArrowLeft, Minus, Plus, ShoppingCart, CalendarCheck, Sparkles } from 'lucide-react';
...
export default function Residential() {
  ...
  return (
    <div className="pt-20 pb-32 bg-white min-h-screen">
      <Helmet>
        <title>Residential Cleaning Services in Dubai | Lucky Crystal Maids</title>
        <meta name="description" content="Top-rated residential cleaning in Dubai. Keep your apartment or villa spotless with our professional weekly, bi-weekly, or monthly maid services. Book online today!" />
      </Helmet>

      {/* Hero */}
      <div className="relative h-[400px] w-full overflow-hidden">
        <img src="https://images.unsplash.com/photo-1581578731548-c64695cc6952?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" alt="Dubai Residential Cleaning" className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-2">Residential Cleaning in Dubai</h1>
            <p className="text-xl text-gray-200 mb-6">Your home, perfectly maintained week after week.</p>
      ...
      <div className="container mx-auto px-6 max-w-6xl mt-16 grid md:grid-cols-3 gap-12">
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:col-span-2">
          <h2 className="text-3xl font-display font-bold text-gray-800 mb-6">Professional Home Cleaning in Dubai</h2>
          <p className="text-gray-600 mb-8 leading-relaxed">Whether you live in a bustling Dubai Marina apartment or a spacious villa in Jumeirah, our residential cleaning service is designed to give you back your free time. We use eco-friendly products and proven techniques to ensure every corner of your home sparkles.</p>
          
          <div className="grid md:grid-cols-2 gap-4 mb-8">
            {['Dusting all surfaces & fixtures', 'Vacuuming carpets & rugs', 'Mopping hard floors', 'Kitchen appliance exteriors', 'Bathroom sanitization', 'Making beds', 'Emptying trash bins'].map((f, i) => (
              <div key={i} className="flex items-start bg-gray-50 p-4 rounded-xl"><CheckCircle className="w-6 h-6 text-crystal-500 mr-3 flex-shrink-0 mt-1" /><span className="text-gray-700">{f}</span></div>
            ))}
          </div>
        </motion.div>
